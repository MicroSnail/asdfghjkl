'''
    ...
    Last modified: 2013/10/3 13:53 by Jialun "Julian" Luo - note: still buggy, actions still needed
'''
...
class WebCrawler:
    ...
    '''
    def getCorrectPrefix_old(self, prefixGiven):
        '''
        @param prefixGiven: the prefix passed to __init__
        @return: if no prefix was passed to __init__ then gets the search domain from the home page url, or else it returns the prefix given
        @note: If a prefix includes http:// then it greatly reduces the number of crawl-able web pages.
...
#         print self.homePage.getChildURLs()
        ...
        @todo: fix this!!
        '''
        maxDistance = self.getFarthestPath(0, self.homePage)[0]
        self.farthestPathLinks.append(self.homePage.getURL())
        return maxDistance
        
        
    def getFarthestPath(self, depth, startPage):
        curDepth = depth
        self.visitedURLs.append(startPage.getURL())
        ...
        for i in range(len(nextLevelPages)):
            ...
            if(nextStartPage.getURL() not in self.visitedURLs):
#                 print
#                 print self.visitedURLs
                
                depthPathPair = self.getFarthestPath(curDepth+1, nextStartPage)
                
                terminalDepthPagePairs[0].append(depthPathPair[0])
                terminalDepthPagePairs[1].append(depthPathPair[1])
            

        
        '''
        The farthest page and its depth
        '''
        maxDepth = max(terminalDepthPagePairs[0])
        maxDepthIndex = terminalDepthPagePairs[0].index(maxDepth)
        farthestPathPage = terminalDepthPagePairs[1][maxDepthIndex]

        self.farthestPathLinks.append(farthestPathPage.getURL())
        return [maxDepth, startPage]
    
    def getMaxDistancePath_discard(self):
        '''
        @todo: fix it
