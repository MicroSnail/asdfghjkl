'''
    ...
    Last modified: 2013/10/3 1:21AM by Jialun "Julian" Luo
'''
...
class WebPage:
    ...
    def setInMaxPath(self, isInMaxPath):
        '''
        @param isInMaxPath: Expect a boolean here
        '''
        self.isInMaxPath = isInMaxPath
        
    def isInMaxPath(self):
        return self.isInMaxPath
...
class WebCrawler:
    ...
    def getMaxDistance(self):
        ...
        print self.homePage.getURL()
        print self.homePage.getChildURLs()
        print 'this is in getMaxDistance()'
                
        return self.getFarthestPath(0, self.homePage)[0]
    
    def getFarthestPath(self, depth, startPage):
        curDepth = depth
        self.visitedLinksCurLv.append(startPage.getURL())
        
        terminalDepthPagePairs = [[],[]] #tuples of the links in deepest directory from this level and their depths
        ...
        for i in range(len(nextLevelPages)):
            
            nextStartPage = nextLevelPages.pop()
            
            print 'visiting'
            print nextStartPage.getURL()
            
            if(nextStartPage.getURL() not in self.visitedLinksCurLv):
                print 'before adding'
                print self.visitedLinksCurLv
                self.visitedLinksCurLv.append(nextStartPage.getURL())
                
                print 'after adding'
                print self.visitedLinksCurLv
                depthPagePair = self.getFarthestPath(curDepth+1, nextStartPage)
                terminalDepthPagePairs[0].append(depthPagePair[0])
                terminalDepthPagePairs[1].append(depthPagePair[1])
        
        maxDepth = max(terminalDepthPagePairs[0])
        maxDepthIndex = terminalDepthPagePairs[0].index(maxDepth)
        pageInFarthestPath = terminalDepthPagePairs[1][maxDepthIndex]
        self.farthestPathLinks.append(pageInFarthestPath.getURL())
        return [maxDepth, pageInFarthestPath]
    ...
    def getMaxDistancePath(self):
        ...
        for i in range(len(allLinks)):
            for j in range(len(allLinks)):
                if(allLinks[j].getURL() == allLinks[i].getURL()):
                    ...
                    if(allLinks[i].getDistanceFromHome() > self.maxDistance):
                        maxDistancePages = []
                        maxDistancePages.append(allLinks[i].getURL())
...
def printActionSummary(crawler):
    ...
    for i in range(len(crawler.farthestPathLinks)):
        print crawler.farthestPathLinks[i]
